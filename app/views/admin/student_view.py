...
from app.serializers.admin.student_serializer import ListStudentSerializer, StudentSerializer
...
class CreateStudentView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"message":"Student created successfully"}, status=status.HTTP_201_CREATED, headers=headers)
        except Exception as e:
            return Response({"message":str(e)}, status=status.HTTP_406_NOT_ACCEPTABLE)

class UpdateDeleteStudentView(generics.RetrieveUpdateDestroyAPIView):
    authentication_classes = [AdminAuthentication]
    serializer_class = StudentSerializer
    queryset = Student.objects.filter(deleted_at__isnull=True)
